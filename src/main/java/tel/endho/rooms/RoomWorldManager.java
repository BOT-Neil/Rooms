...
public class RoomWorldManager {
    ...
    public void migratePlot(Player player) throws SQLException, IOException, WorldAlreadyExistsException {
        ...
        if(plot!=null){
            ...
            if(!plot.isMerged()){
                ...
                System.out.println("plotowneruuid: "+plot.getOwner().toString());
                System.out.println("playeruuid: "+player.getUniqueId());
        ...
        if (plot!=null&&!plot.isMerged()&&player.getUniqueId().equals(plot.getOwner())) {
            ...
            //Location location= new Location(Bukkit.getWorld(worlduuid.toString()),1,255,1);
    ...
    public void createWorld(String worldtype, Player player) {
        try {
            ...
            Rooms.getPlugin().getLogger().info("test123: "+ Biome.CRIMSON_FOREST.toString() );
            if(worldtype.equals("nether")){
                properties.setValue(SlimeProperties.DEFAULT_BIOME,"minecraft:crimson_forest");
                Rooms.getPlugin().getLogger().info("propbiome: "+ properties.getValue(SlimeProperties.DEFAULT_BIOME));
            }
            properties.setValue(SlimeProperties.DIFFICULTY, "normal");
    ...
    public void genNether(RoomWorld roomWorld, Player player) {
        ...
        try {
            ...
            properties.setValue(SlimeProperties.SPAWN_Y, Rooms.configs.getGeneralConfig().getInt("spawnheight"));
            properties.setValue(SlimeProperties.SPAWN_Z, 1);
            SlimeWorld world = plugin.createEmptyWorld(sqlLoader, roomWorld.getWorldUUID().toString()+"rmnether", false, properties);
            // This method must be called synchronously
            plugin.generateWorld(world);
            BukkitRunnable r = new BukkitRunnable() {
                @Override
    ...
    public void TpOrLoadHouseWorld(Player p, UUID uuid) throws CorruptedWorldException, NewerFormatException, WorldInUseException, UnknownWorldException, IOException {
                ...
                if(Rooms.configs.getStorageConfig().getBoolean("redislobby")&&Rooms.redis.isLoaded()){
                    ...
                    if (RoomWorlds.isRoomWorld(uuid)) {
                        ...
                        if (Bukkit.getWorld(roomWorld.getWorldUUID().toString()) != null) {
                            Location location = new Location(Bukkit.getWorld(uuid.toString()), roomWorld.getSpawnX().doubleValue(), roomWorld.getSpawnY().doubleValue(), roomWorld.getSpawnZ().doubleValue());
                            p.teleport(location);
                        } else {
    ...
    public void unloadRoomWorld(RoomWorld roomWorld){
        ...
        try {
            ...
            System.out.println("system path: "+Rooms.getPlugin().getDataFolder().getAbsolutePath());//system path: /home/creative/CreativeEU1/plugins/Rooms
            File bob = new File(Rooms.getPlugin().getDataFolder().getParent()+"/WorldGuard/worlds/"+ roomWorld.getWorldUUID().toString());
            bob.deleteOnExit();
    ...
    public void deleteRoomWorld(RoomWorld roomWorld) throws UnknownWorldException, IOException {
        Rooms.getPlugin().getLogger().info("");
        World world = Bukkit.getWorld(roomWorld.getWorldUUID().toString());
        assert world != null;
    ...
    public void loadWorld(RoomWorld roomWorld) throws CorruptedWorldException, NewerFormatException, WorldInUseException, UnknownWorldException, IOException {
        SlimePropertyMap properties = new SlimePropertyMap();
        properties.setValue(SlimeProperties.WORLD_TYPE, "flat");
        ...
        System.out.println("allowanimals: "+properties.getValue(SlimeProperties.ALLOW_ANIMALS));
        //properties.setValue(SlimeProperties.ALLOW_ANIMALS, Rooms.configs.getGeneralConfig().getBoolean("spawnanimals"));
        //properties.setValue(SlimeProperties.ALLOW_MONSTERS, Rooms.configs.getGeneralConfig().getBoolean("spawnmonsters"));

        SlimeWorld world = plugin.loadWorld(sqlLoader, roomWorld.getWorldUUID().toString(),false,properties);
        plugin.generateWorld(world);
        Objects.requireNonNull(Bukkit.getWorld(roomWorld.getWorldUUID().toString())).setGameRule(GameRule.DO_MOB_SPAWNING,false);
        Objects.requireNonNull(Bukkit.getWorld(roomWorld.getWorldUUID().toString())).setGameRule(GameRule.DO_FIRE_TICK,false);
