...
public class GlobalRoomsMenu extends PaginatedMenu {
    ...
    public void setMenuItems() {
        ...
        GlobalRoomWorlds.getGlobalRoomWorlds().values().forEach(globalRoomWorld -> {
            ...
            /*if(!globalRoomWorld.getLastserver().equals(Rooms.configs.getStorageConfig().getString("bungeeservername"))){
        ...
        if(!globalrooms.isEmpty()) {
            for(int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if(index >= globalrooms.size()) break;
                if (globalrooms.get(index) != null){
                    ...
                    playerMeta.getPersistentDataContainer().set(new NamespacedKey(Rooms.instance, "uuid"), PersistentDataType.STRING, globalrooms.get(index).getWorldUUID().toString());
