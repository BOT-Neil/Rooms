...
public class BRKVisitRooms {
  public void makemenu(Player player) throws SQLException {
    ...
    for (int i = 0; i < playerhouses.size(); i++) {
      stockList.add(i + ". " + playerhouses.get(i).getPreset());
      index.set(i);
    }
    ...
    customForm.setResponseHandler(responseData -> {
       ...
       * try {
       * Rooms.housemanager.TpOrLoadHouseWorld(player,playerhouses.get(dropclick0).
       * getWorldUUID());
       * } catch (CorruptedWorldException | NewerFormatException | WorldInUseException
       * | UnknownWorldException | IOException e) {
       * e.printStackTrace();
       * }
