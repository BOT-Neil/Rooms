...
public class GlobalRoomWorlds {
    //private static Map<UUID, ArrayList<RoomWorld>> offlineRoomWorldmap = new HashMap<>();//player uuid
    ...
    public static boolean isOnAnotherServer(UUID uuid) {
        ...
        if(globalRoomWorldHashMap.get(uuid)==null){
            ...
            System.out.println("lastserver: "+globalRoomWorldHashMap.get(uuid).lastserver);
            System.out.println("currentserver: "+Rooms.configs.getStorageConfig().getString("bungeeservername"));
            return !globalRoomWorldHashMap.get(uuid).lastserver.equals(Rooms.configs.getStorageConfig().getString("bungeeservername"));
        }}catch(Exception e){
            return false;
        }
        //return !globalRoomWorldHashMap.get(uuid).lastserver.equals(Rooms.configs.getMySQLConfig().getString("bungeeservername"));
    }
}
